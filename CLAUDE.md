...
- **`dashboard/index.html`** — WC3-themed local web dashboard (served by `peon dashboard`). Single HTML file with embedded CSS/JS, polls `.state.json` via a local Python HTTP server on port 19997.
...
- **Buildings:** Spend gold/lumber via `peon build <name>` to construct structures that unlock perks (combos, fatigue pause, idle thoughts, on-demand taunts).
...
All game state lives in `.state.json` under `stats`, `economy`, `buildings`, `combo_count`, `activity_log`. Config under `game` key in `config.json`.
