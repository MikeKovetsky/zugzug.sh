...
| 9 | 熊猫酒仙 | 熊猫人 | 10,000 | `wc3_brewmaster` |
| 10 | 大法师 | 人族 | 25,000 | `dota2_invoker` |
| 11 | 凯恩·血蹄 | 牛头人 | 75,000 | `wow-tauren` |
| 12 | 剑圣 | 兽族 | 150,000 | `dota2_phantom_lancer` |
| 13 | 萨尔 | 兽族 | 250,000 | `zugzug` |
| 14 | 克尔苏加德 | 亡灵 | 500,000 | `wc3_lich` |
| 15 | ??? | ??? | 1,000,000 | ??? |

你的等级决定一切：听到的语音、通知中的头像、仪表板中显示的角色。后期英雄（10-14 级）借用 Dota 2 语音包，这些角色与其魔兽争霸 III 对应英雄是显而易见的匹配 — 大法师借用祈求者的「冰雷火」多咒语吟诵，剑圣获得幻影长矛手的镜像台词（`我是一人，我亦千军`），与第 7 级巫医使用 Dota 2 巫医语音包的方式相同。据说存在一个秘密的第 15 级。目前还没有人达到过。
...
建造**奥术圣殿**（7,500g / 3,000l）后即可解锁 `peon brew <item>`：用金币 + 木材酿造伤害消耗品，无需依赖 BOSS 掉落 RNG。配方按进度分层 — 闪电链与死亡缠绕为即时酿造；末日要排队 3 个完成任务；传说级（**夺魂者** 200k 伤害、**双子蚀** 500k 伤害）还需要消耗基尔加丹之魂碎片（每击杀基尔加丹掉落 1 个），是挑战巫妖王前的预备弹药。
